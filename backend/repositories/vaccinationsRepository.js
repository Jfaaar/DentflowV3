...
function fromDb(row) {
  ...
  return {
    ...
    doseNumber: row.dose_number ?? undefined,
...
async function list(db, { page = 0, pageSize = 100, patientId }) {
  ...
  const dataSQL = `SELECT * FROM vaccinations ${whereSQL} ORDER BY administered_date DESC LIMIT $${limitIdx} OFFSET $${offsetIdx}`;
  const countParams = params.slice(0, params.length - 2);
  const countSQL = `SELECT COUNT(*)::int AS count FROM vaccinations ${whereSQL}`;
...
async function get(db, id) {
  const r = await db.query(`SELECT * FROM vaccinations WHERE id = $1`, [id]);
...
async function create(db, input, clinicId) {
  ...
  const r = await db.query(
    `INSERT INTO vaccinations (${cols.join(', ')}) VALUES (${placeholders}) RETURNING *`,
    vals,
  );
  return fromDb(r.rows[0]);
}
...
async function update(db, id, patch) {
  ...
  params.push(id);
  const r = await db.query(
    `UPDATE vaccinations SET ${sets.join(', ')} WHERE id = $${params.length} RETURNING *`,
    params,
  );
  return fromDb(r.rows[0]);
}
